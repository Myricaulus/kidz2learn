...
public sealed class SkillMasteryStore(IndexedDb skillDb)
{
    ...
    public async Task Adjust(string skillId, Difficulty difficulty, int timeMs, Kompetenzniveau specificTaskHistory, bool success)
    {
        float delta;
        if(success)
        {
        ...
        var s = await _store.GetItemAsync<SkillState>(skillId)?? new SkillState { Id = skillId, EffectiveDifficulty = SkillRegistry.All[skillId].Difficulty};
        s.Attempts++;
        if (success) 
        {
            s.Success++;
        ...
        specificTaskHistory.CountLastFalschRow(); // return 0 if last attempt was a success, otherwise how many of the last attempts were wrong in a row
        specificTaskHistory.CountLastRichtigRow(); // return 0 if last attempt has failed, otherwise how many of the last attempts were correct in a row
